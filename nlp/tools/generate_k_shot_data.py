...
def load_datasets(data_dir, tasks):
    ...
    for task in tasks:
        ...
        elif task in ["imdb", "ag", "yelp"]:
            # jsonl or txt
            dataset = {}
            dirname = os.path.join(data_dir, task)
            splits = ["train", "test"]
            postfix = "jsonl" if task in ["imdb", "ag"] else "txt"
            for split in splits:
                filename = os.path.join(dirname, f"{split}.{postfix}")
                with open(filename, "r") as f:
                    lines = f.readlines()
                dataset[split] = lines
            datasets[task] = dataset
...
def main():
    ...
    parser.add_argument(
        "--seed", type=int, nargs="+", default=[109, 74, 53, 44, 186, 66, 13, 21, 42, 87, 100], help="Random seeds"
    )
    ...
    for seed in args.seed:
        ...
        for task, dataset in datasets.items():
            ...
            setting_dir = os.path.join(task_dir, "sampled_datasets", f"seed_{seed}", f"{args.k}")
            ...
            if task in ["MNLI", "MRPC", "QNLI", "QQP", "RTE", "SNLI", "SST-2", "STS-B", "WNLI", "CoLA"]:
                ...
                postfix = "jsonl" if task in ["imdb", "ag"] else "txt"
                with open(os.path.join(setting_dir, f"test.{postfix}"), "w") as f:
            ...
            if task in ["MNLI", "MRPC", "QNLI", "QQP", "RTE", "SNLI", "SST-2", "STS-B", "WNLI", "CoLA"]:
                ...
                with open(os.path.join(setting_dir, name), "w") as f:
                    ...
                    #         f.write(line)
                    for line in train_lines[k : k * dev_rate]:
                        f.write(line)
            elif task in ["imdb", "ag", "yelp"]:
                postfix = "jsonl" if task in ["imdb", "ag"] else "txt"
                with open(os.path.join(setting_dir, f"train.{postfix}"), "w") as f:
                    for line in train_lines[:k]:
                        f.write(line)
                with open(os.path.join(setting_dir, f"dev.{postfix}"), "w") as f:
                    dev_rate = 11 if "10x" in args.mode else 2
                    for line in train_lines[k : k * dev_rate]:
                        f.write(line)
            else:
